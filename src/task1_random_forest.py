# Rebuilt from Task1 (3).ipynb: Task 1 Random Forest.
...
cv_scores = cross_val_score(
    baseline_pipeline, X_train, y_train,   # pass raw X_train, not pre-scaled
...
final_model.fit(X, y)
print(f"\n  Final RF retrained on all {len(y)} Task 1 training samples.")

# ─────────────────────────────────────────────────────────────────────────────
# 6b. MODEL COMPARISON: Baseline RF vs Tuned RF
# ─────────────────────────────────────────────────────────────────────────────

# Baseline CV mean accuracy (already computed in section 4)
baseline_mean = cv_scores.mean()
baseline_std  = cv_scores.std()

# Tuned RF: best CV score from GridSearchCV
tuned_cv_mean = grid_search.best_score_
# Std of the best param combination across folds
best_idx      = grid_search.best_index_
tuned_cv_std  = grid_search.cv_results_["std_test_score"][best_idx]

# Final tuned RF validation accuracy (from section 6)
# val_accuracy is a single number so no std
...
ax.set_title("Task 1 — Baseline RF vs Tuned RF")
ax.set_ylim(0, 1)
ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1))
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "task1_rf_model_comparison.png", dpi=150)
plt.close()
print(f"  Model comparison chart saved to: {OUTPUT_DIR / 'task1_rf_model_comparison.png'}")

# ─────────────────────────────────────────────────────────────────────────────
# 7. CONFUSION MATRIX  →  task1_rf_confusion_matrix.png
# ─────────────────────────────────────────────────────────────────────────────
...
ax.set_title(f"Task 1 — Random Forest Confusion Matrix\n"
             f"Validation accuracy: {val_accuracy:.2%}", fontsize=13)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "task1_rf_confusion_matrix.png", dpi=150)
plt.close()
print(f"  Confusion matrix saved to: {OUTPUT_DIR / 'task1_rf_confusion_matrix.png'}")

# ─────────────────────────────────────────────────────────────────────────────
# 8. CV ACCURACY BAR CHART  →  task1_rf_cv_accuracy.csv  +  .png
# ─────────────────────────────────────────────────────────────────────────────

cv_results_df = pd.DataFrame(grid_search.cv_results_)

# Keep only the columns we need and compute mean ± std across folds
...
cv_summary.to_csv(OUTPUT_DIR / "task1_rf_cv_accuracy.csv", index=False)
print(f"  CV accuracy table saved to: {OUTPUT_DIR / 'task1_rf_cv_accuracy.csv'}")

# Bar chart: top 20 hyperparameter combinations
top_n = min(20, len(cv_summary))
top   = cv_summary.head(top_n)

# Build readable x-axis labels from parameter columns
...
ax.set_title("Task 1 — Random Forest GridSearchCV: Top Hyperparameter Combinations")
ax.yaxis.set_major_formatter(mticker.PercentFormatter(xmax=1))
ax.set_ylim(0, 1)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "task1_rf_cv_accuracy.png", dpi=150)
plt.close()
print(f"  CV accuracy bar chart saved to: {OUTPUT_DIR / 'task1_rf_cv_accuracy.png'}")

# ─────────────────────────────────────────────────────────────────────────────
# 9. FEATURE IMPORTANCE PLOT  →  task1_rf_best_features.png
#    Ranked by feature CATEGORY (averaged importance across all features
#    within each category) rather than individual features.
# ─────────────────────────────────────────────────────────────────────────────
...
def get_category(col_name):
    if col_name.startswith("color_"):
        ...
        # texture_vec = [multiscale_lbp (54), haralick (13), gabor (24), hsv (48)]
        # Map index within texture block to sub-category
...
# ── Compute mean importance per category ──────────────────────────────────────
category_importance = {}
...
ax.set_title("Task 1 — Random Forest: Top 5 Most Important Feature Categories")
for i, (name, val) in enumerate(zip(top5_names[::-1], top5_vals[::-1])):
    ax.text(val + 0.0001, i, f"{val:.4f}", va="center", fontsize=9)
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "task1_rf_best_features.png", dpi=150)
plt.close()
print(f"  Feature category importance plot saved to: {OUTPUT_DIR / 'task1_rf_best_features.png'}")

# ─────────────────────────────────────────────────────────────────────────────
# 10. CLASSIFICATION REPORT  →  task1_rf_classification_report.csv
# ─────────────────────────────────────────────────────────────────────────────

report_dict = classification_report(y_val, y_pred, digits=4, output_dict=True)
report_df   = pd.DataFrame(report_dict).transpose().reset_index()
report_df.rename(columns={"index": "class"}, inplace=True)
report_df.to_csv(OUTPUT_DIR / "task1_rf_classification_report.csv", index=False)
print(f"  Classification report saved to: {OUTPUT_DIR / 'task1_rf_classification_report.csv'}")

# ─────────────────────────────────────────────────────────────────────────────
# 11. TEST PREDICTIONS  →  task1_rf_submission.csv
# ─────────────────────────────────────────────────────────────────────────────
...
submission.to_csv(OUTPUT_DIR / "task1_rf_submission.csv", index=False)

print(f"  Saved to: {OUTPUT_DIR / 'task1_rf_submission.csv'}")
